...
function info_add() {
    var layIndex = layer.open({
        ...
        maxmin: true,
        area: ['700px', '600px'],
        content: [contextPath + '/productController/addPage.html', 'no'],
    });
}
...
function info_view(data) {
    ...
    var layIndex = layer.open({
        ...
        maxmin: true,
        area: ['500px', '400px'],
        content: [contextPath + '/productController/detail.html?' + $.param({uId: data[0]['uId']})]
    });
}
...
function info_delete(data) {
    ...
    layer.confirm('确定删除吗?', {icon: 3, title: '提示'},
        function (index) {
            var mask = layer.load();
            $.ajax({
                url: "/productController/deleteProductById.json",
                type: "POST",
